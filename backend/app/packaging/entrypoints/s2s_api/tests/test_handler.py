...
def test_empty_resource_actions_return_valid_json_proxy_body(
    monkeypatch,
    mocked_dependencies,
    lambda_context,
    client_event,
    ...
    mocked_dependencies.recipe_domain_qry_srv.get_recipe.return_value = mock.Mock(status=recipe.RecipeStatus.Created)

    response = load_handler(monkeypatch, mocked_dependencies).handler(
        client_event(method, path, body, scopes=[scope]), lambda_context
    )
